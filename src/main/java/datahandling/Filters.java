...
public class Filters {
    ...
    public List<Map<String, String>> WithIdentifier(List<Map<String, String>> CSV, String identifier) {
        // With a specified identifier "identifier"
        return CSV.stream()
                .filter(dico -> dico.get("Identifiant").contains(identifier))
                .collect(Collectors.toList());
    }
    public List<Map<String, String>> WithName(List<Map<String, String>> CSV, String name) {
        // With a specified name "name"
        return CSV.stream()
                .filter(dico -> dico.get("Nom").contains(name))
                .collect(Collectors.toList());
    }
    public List<Map<String, String>> AtPointRGF(List<Map<String, String>> CSV, int X, int Y, int range){
    ...
    public List<Map<String, String>> AtPointWGS(List<Map<String, String>> CSV, int X, int Y, int range){
        List<Map<String, String>> data = new ArrayList<>();
    ...
    public List<Map<String, String>> AtDate(List<Map<String, String>> CSV, int year) {
        // At a specified Date "year"
        return CSV.stream()
                .filter(dico -> dico.get("Date (AAAA/MM/JJ)").contains(String.format("%4d", year)))
                .collect(Collectors.toList());
    }
    public List<Map<String, String>> BetweenDate(List<Map<String, String>> CSV, int yearMin, int yearMax){
    ...
    public List<Map<String, String>> AtRegion(List<Map<String, String>> CSV, String region){
        // At a specified region "region"
        return CSV.stream()
                .filter(dico -> dico.get("Région épicentrale").contains(region))
                .collect(Collectors.toList());
    }
    public List<Map<String, String>> WithQIE(List<Map<String, String>> CSV, String QIE){
        // At a specified QIE "Qualité intensité épicentrale"
        return CSV.stream()
                .filter(dico -> Objects.equals(dico.get("Qualité intensité épicentrale"), QIE))
                .collect(Collectors.toList());
    }
    public List<Map<String, String>> WithChoc(List<Map<String, String>> CSV, String Choc){
